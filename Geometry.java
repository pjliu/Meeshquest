	...
	public int compareTo(Geometry o) {
		if (this.isCity()) {
			...
			if (o.isCity()) {
				...
				// o is a road
				if (((Road) this).getStart().getName().compareTo(((Road) o).getStart().getName()) == 0) {
					// start names are the same so compare end names
					return ((Road) this).getEnd().getName().compareTo(((Road) o).getEnd().getName());
				} else {
					/* start names are different; compare start names */
					return ((Road) this).getStart().getName().compareTo(((Road) o).getStart().getName());
				}
			}
		}
	}
